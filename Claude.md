Looking at the error, the issue is that `HdfResultsMesh.get_mesh_min_ws()` internally calls `HdfMesh.get_mesh_cell_points()` which creates a GeoDataFrame with CRS, triggering the pyproj conflict.

I'll update both the min and max WSE algorithms to use direct HDF access, following the pattern already successfully used in other algorithms like `alg_load_2d_max_iter_points.py`.

## 1. Updated `alg_load_2d_min_wse_points.py`:
...
## 2. Updated `alg_load_2d_max_wse_points.py`:
...
class Load2DMaximumWaterSurfacePointsAlgorithm(QgsProcessingAlgorithm):
    """
    Loads maximum water surface elevation points from a HEC-RAS 2D results HDF file.
    
    This tool extracts the maximum water surface elevation at each 2D mesh cell
    center over the entire simulation period, and loads them as a point
    vector layer in the "Benefit Area Analysis" group in QGIS.
    """
    INPUT_HDF = 'INPUT_HDF'
    ...
    def displayName(self):
        return 'Load 2D Max Water Surface Elevations at Cell Centers'
    ...
    def postProcessAlgorithm(self, context: QgsProcessingContext, feedback):
        ...
        if self.OUTPUT_LAYER in results:
            ...
            if layer:
                ...
                success = move_layer_to_benefit_area_group(layer.id())
                if success:
                    feedback.pushInfo(f"Maximum WSE Points layer moved to 'Benefit Area Analysis' group")
                else:
                    feedback.pushInfo("Could not move layer to group (layer created successfully)")
    ...
    def _get_max_wse_direct(self, hdf_path, feedback=None):
        """
        Direct HDF access to get maximum water surface elevations and cell centers.
        Bypasses ras-commander GeoDataFrame creation to avoid CRS conflicts.
        """
        all_data = []
        proj_wkt = None
        ...
        try:
            ...
            raise QgsProcessingException(f"Error reading max WSE data directly from HDF: {e}")
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        try:
            # Use direct HDF access to avoid CRS conflicts
            feedback.pushInfo("Using direct HDF access to avoid CRS conflicts...")
            proj_wkt, raw_df, wkt_geometries = self._get_max_wse_direct(hdf_path, feedback)
        ...
        fields.append(QgsField("mesh_name", QMetaType.Type.QString))
        fields.append(QgsField("cell_id", QMetaType.Type.Int))
        fields.append(QgsField("max_wse", QMetaType.Type.Double))
        ...
        for i, (_, row) in enumerate(gdf.iterrows()):
            ...
            feature.setAttribute("mesh_name", row['mesh_name'])
            feature.setAttribute("cell_id", int(row['cell_id']))
            feature.setAttribute("max_wse", float(row['max_wse']))
...
1. **Direct HDF Access**: Both algorithms now use the `_get_max_wse_direct()` and `_get_min_wse_direct()` methods to read data directly from the HDF file, completely bypassing the ras-commander methods that trigger CRS conflicts.

2. **Manual Data Assembly**: The algorithms manually read:
   - Cell center coordinates from the geometry section
   - Summary water surface data from the results section
   - Combine them into a DataFrame with WKT geometry strings

3. **Time Conversion**: Uses `HdfUtils.convert_timesteps_to_datetimes()` to properly convert the time values from days to datetime strings.

4. **Consistent Pattern**: Both algorithms follow the same pattern as other successfully working algorithms, ensuring no pyproj conflicts can occur.

This approach ensures that no CRS-aware geometry objects are created in the user's Python environment, completely avoiding the pyproj version conflict.